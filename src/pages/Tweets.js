...
export default function Tweets() {
  ...
  const wordClouds = selectedValue.map((element, index) => {
    return (
      <Grid item xs={6}>
        <Wordcloud username={element.username} label={element.label} key={index} />
      </Grid>
    );
  })
  return (
    <Container>
      ...
      <Typography variant="subtitle" gutterBottom>
        A word cloud visualizes the  relative frequency distribution of words in certain text. i.e which
        are the most frequent word used by the user.
      </Typography>
      <div style={{ height: '5px' }}></div>

      <Typography variant="subtitle" gutterBottom>
        Choose the user from the dropdown to see the word cloud of the perticular user.
      </Typography>
